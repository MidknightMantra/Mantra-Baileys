...
export const makeMessagesSocket = (config: SocketConfig) => {
        ...
        const sendReaction = async (jid: string, key: WAMessageKey, emoji: string) => {
                return sendMessage(jid, { react: { key, text: emoji } })
        }

        const removeReaction = async (jid: string, key: WAMessageKey) => {
                return sendMessage(jid, { react: { key, text: '' } })
        }

        const forwardMessages = async (jid: string, messages: WAMessage[], options: MiscMessageGenerationOptions = {}) => {
                const results: WAMessage[] = []
                for (const msg of messages) {
                        const sent = await sendMessage(jid, { forward: msg, force: true }, options)
        ...
        const sendAlbumMessage = async (
                jid: string,
                mediaList: { media: import('../Types').WAMediaUpload; caption?: string; mimetype?: string; type?: 'image' | 'video' }[],
                options: MiscMessageGenerationOptions = {}
        ): Promise<WAMessage[]> => {
                if (mediaList.length === 0) throw new Error('Album must contain at least one media item')
                const sentMessages: WAMessage[] = []
                for (let i = 0; i < mediaList.length; i++) {
                        const item = mediaList[i]!
                        const caption = i === 0 ? item.caption : undefined
                        const mediaType = item.type ?? 'image'
                        const content = mediaType === 'video'
                                ? { video: item.media, caption, mimetype: item.mimetype }
                                : { image: item.media, caption, mimetype: item.mimetype }
                        const sent = await sendMessage(jid, content as AnyMessageContent, options)
        ...
        const sendTyping = async (jid: string, durationMs?: number) => {
        ...
        const sendStatusStory = async (content: AnyMessageContent, statusJidList?: string[]) => {
                return sendMessage('status@broadcast', content, { statusJidList })
        }

        const pinMessage = async (jid: string, key: WAMessageKey, time: 86400 | 604800 | 2592000 = 604800) => {
        ...
        const unpinMessage = async (jid: string, key: WAMessageKey) => {
                return sendMessage(jid, {
                        pin: key,
