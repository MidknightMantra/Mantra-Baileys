...
export const makeGroupsSocket = (config: SocketConfig) => {
        ...
        return {
                ...
                groupGetInviteLink: async (jid: string) => {
                        const result = await groupQuery(jid, 'get', [{ tag: 'invite', attrs: {} }])
                        const inviteNode = getBinaryNodeChild(result, 'invite')
                ...
                groupBulkParticipantsUpdate: async (
                        jid: string,
                        participants: string[],
                        action: ParticipantAction,
                        onProgress?: (done: number, total: number, jid: string, status: string) => void
                ) => {
                        const results: { jid: string; status: string }[] = []
                                        ...
                                        const result = await groupQuery(jid, 'set', [
                                                {
                                                        tag: action,
                                                        attrs: {},
                                                        content: [{ tag: 'participant', attrs: { jid: p } }]
                                                }
                                        ])
                                        const node = getBinaryNodeChild(result, action)
                                        const affected = getBinaryNodeChildren(node, 'participant')
                                        const status = affected[0]?.attrs?.error ?? '200'
                                        results.push({ jid: p, status })
                                        onProgress?.(i + 1, participants.length, p, status)
                                } catch (err) {
                                        results.push({ jid: p, status: 'error' })
                                        onProgress?.(i + 1, participants.length, p, 'error')
                                }
                        }
                ...
                groupGetAllAdmins: async (jid: string) => {
